/* ============================================
   HERO POSTER v4 - Pikachu Side-by-Side
   - Pikachu LEFT, title RIGHT, same height
   - Game link cards with legendary Pokemon
   ============================================ */
(function(){
  'use strict';
  ...
  // Legendary Pokemon for each game
  var GAME_LINKS = [
  ...
  css.textContent = '\
.hero-poster{position:relative;z-index:1;max-width:1200px;margin:0 auto;padding:40px 24px 20px;display:flex;align-items:center;justify-content:center;gap:32px;min-height:360px}\
.hero-poster::before{content:"";position:absolute;inset:0;background:radial-gradient(ellipse at 30% 50%,rgba(241,196,15,.08) 0%,transparent 60%),radial-gradient(ellipse at 70% 30%,rgba(231,76,60,.06) 0%,transparent 50%);z-index:0;pointer-events:none}\
\
.hero-pikachu{flex-shrink:0;position:relative;z-index:2;animation:pikaIn .8s both}\
.hero-pikachu img{width:220px;height:220px;image-rendering:pixelated;filter:drop-shadow(0 0 30px rgba(241,196,15,.35));transition:transform .4s,filter .4s}\
.hero-pikachu img:hover{transform:scale(1.1) rotate(-3deg);filter:drop-shadow(0 0 50px rgba(241,196,15,.6))}\
@keyframes pikaIn{from{opacity:0;transform:translateX(-30px) scale(.7)}to{opacity:1;transform:translateX(0) scale(1)}}\
\
.hero-text{position:relative;z-index:2;max-width:560px;animation:textIn .8s .2s both}\
@keyframes textIn{from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)}}\
.hero-tag{display:inline-block;background:rgba(231,76,60,.15);border:1px solid rgba(231,76,60,.4);border-radius:20px;padding:4px 16px;font-size:.75rem;color:var(--red,#e74c3c);text-transform:uppercase;letter-spacing:2px;margin-bottom:14px;font-weight:600}\
.hero-title{font-family:"Chakra Petch",sans-serif;font-size:clamp(2.2rem,5.5vw,3.6rem);font-weight:700;line-height:1.1;margin-bottom:14px}\
...
.hero-sub{color:var(--text2,#95a5a6);font-size:.95rem;line-height:1.6;margin-bottom:20px}\
.hero-cta{display:inline-flex;align-items:center;gap:8px;background:var(--red,#e74c3c);color:#fff;border:none;padding:12px 32px;border-radius:8px;font-family:"Outfit",sans-serif;font-size:1rem;font-weight:600;cursor:pointer;transition:all .3s;box-shadow:0 4px 20px rgba(231,76,60,.4);text-decoration:none}\
.hero-cta:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(231,76,60,.5);background:#c0392b}\
.hero-cta svg{width:18px;height:18px}\
\
.game-cards{position:relative;z-index:2;display:flex;justify-content:center;gap:8px;flex-wrap:wrap;max-width:1200px;margin:16px auto 0;padding:0 24px;animation:fadeUp .6s .6s both}\
...
\
.poster-stats{position:relative;z-index:2;display:flex;justify-content:center;gap:28px;margin:20px auto 10px;padding:0 24px;max-width:1200px;animation:fadeUp .6s .8s both}\
...
.hero-poster{flex-direction:column;text-align:center;padding:24px 16px 16px;gap:16px;min-height:auto}\
.hero-pikachu img{width:140px;height:140px}\
...
Start Tracking\
    ...
    // Game cards with legendary Pokemon
